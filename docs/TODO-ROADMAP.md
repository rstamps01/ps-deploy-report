...
**Last updated:** 2026-08-01 — **v1.6.0 released** (tag `v1.6.0`); branch protection on `main`. Pipeline tails closed (test-reliability CI wiring + secrets registry). **M6 started in-repo:** project-manifest adapter (`scripts/pipeline_manifest.py`, skills source commands from `.cursor/pipeline.yml`) + ADF extraction spec (`docs/development/ADF-EXTRACTION.md`). Remaining M6 (deferred, needs new `agentic-dev-framework` repo): core extraction, framework versioning, bootstrap/adoption flows, doctor, handbook. **v1.6.0** development is segmented on `develop` (baseline commit `e48e581`) — committed + documented, **not yet released** (no `v1.6.0` tag); it will ship via the new `prepare-release`/`ship-release` path. See [`PROJECT-STATUS.md`](PROJECT-STATUS.md) for the full snapshot and [`DECISIONS.md`](DECISIONS.md) for the decision log.
