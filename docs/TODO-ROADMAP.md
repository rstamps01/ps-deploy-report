...
**Last updated:** 2026-08-01 — **v1.6.0 released**; branch protection on `main`. Pipeline tails closed. **M6 Phase B done:** the portable core was extracted to [`rstamps01/agentic-dev-framework`](https://github.com/rstamps01/agentic-dev-framework) v0.1.0 (private) — 18 core skills, hooks, location-independent adapter, templates, `HANDBOOK.md`, dogfooding its own gate; this repo is ADF's first adopter. Remaining M6 (Phase C–E): bootstrap + adoption flows + tiers, `framework-doctor`, pilot on VAST Plan Analyzer. **v1.6.0** development is segmented on `develop` (baseline commit `e48e581`) — committed + documented, **not yet released** (no `v1.6.0` tag); it will ship via the new `prepare-release`/`ship-release` path. See [`PROJECT-STATUS.md`](PROJECT-STATUS.md) for the full snapshot and [`DECISIONS.md`](DECISIONS.md) for the decision log.
