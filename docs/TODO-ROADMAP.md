...
## Planned — Reporter-tile `vnetmap` heterogeneous-fleet parity

*Follow-up surfaced by the RM-14 triage pass: the Reporter-tile `vnetmap` still takes the legacy candidate-sweep path because `_run_report_job` populates `switch_password_candidates` but not `switch_password_by_ip`, so `--multiple-passwords` is never engaged.*

| ID    | Item | Priority | Status | Notes |
|-------|------|----------|--------|-------|
| RM-15 | **Pre-probe switches once in `_run_report_job` and feed `VnetmapWorkflow` a `switch_password_by_ip` map.** `VnetmapWorkflow` already has a Fast path (`_build_multiple_passwords_cmd` + `--multiple-passwords`) that feeds each switch IP its correct password via a heredoc map, unlocking heterogeneous fleets where e.g. two leaves are on `Vastdata1!` and a spare is on `VastData1!`. The Test Suite tile engages it because `OneShotRunner._validate_switch_ssh` runs an upfront probe that populates `password_by_ip`; the Reporter tile doesn't run that probe, so the Fast path is skipped and the legacy single-`-p` candidate sweep runs (which can only succeed on homogeneous fleets). Lift the existing `HealthChecker._probe_switch_password` (RM-13) or the `OneShotRunner` probe into a shared helper and invoke it once at the top of `_run_report_job` — before the VnetmapWorkflow handoff — so the Reporter-tile `vnetmap` gets the same `switch_password_by_ip` map the Test Suite tile builds. Expected win: the Reporter-tile `vnetmap` succeeds on heterogeneous fleets in a single invocation instead of cycling through 3–4 candidate sweeps (or failing outright when no single password works). | High (parity + correctness) | Planned | Targets: `src/app.py::_run_report_job` (probe call, `switch_password_by_ip` into `VnetmapWorkflow.set_credentials`), possibly new `src/utils/switch_ssh_probe.py` to host the shared probe. Tests: an `/generate` integration test that asserts `VnetmapWorkflow` sees `switch_password_by_ip` when Autofill is on. |
