...
| TPM-1 | **Teleport (tsh) connection mode + Connection Settings UI redesign.** Add a third connection method alongside Tech Port and VMS HTTPS that tunnels the cluster API (443) and CNode SSH (22) through `tsh ssh -L` dual port forwards so reports and `vnetmap`/port mapping work against Teleport-only clusters. Redesign the Connection Settings tile: three-way connection-mode radios (Tech Port / VMS Mgmt / Teleport) replacing the Advanced-menu Tech Port checkbox, Autofill Passwords promoted to a toggle (ON locks defaults / OFF keeps usernames + clears passwords), and Teleport node/user fields shown only in Teleport mode, persisted in profiles. | High | Done (2026-06-19); **Beta exit Done (2026-09-25)** | New `src/utils/teleport_tunnel.py` (`TeleportTunnel` w/ preflight + dual `-L` forwards); `port`/`jump_port` threaded through `ssh_adapter`, `tool_manager`, `script_runner` (`set_ssh_port`); `ssh_host`/`ssh_port` honored by `vnetmap_workflow`, `external_port_mapper`, `switch_config_workflow`, `health_checker`, and one-shot (`_setup_teleport_tunnel`, `_validate_teleport_tunnel`). `app.py` `generate_start`/`_run_report_job`/`/api/discover` launch the right tunnel per mode. `frontend/templates/reporter.html` UI + CSS; `teleport` config block in `config.yaml.template`. Tests: `tests/test_teleport_tunnel.py` (new) + `tests/test_ssh_adapter.py` explicit-port cases. flake8/black clean. **Shipped as Beta for v1.5.8**; **2026-09-25:** Beta Feature badge and Beta labeling removed from current UI/docs after field validation (historical CHANGELOG / release notes retained). |
...
6. **Teleport beta exit (TPM-1/TPM-2 follow-up):** Done (2026-09-25) — Beta flag removed from UI and current docs after field validation.
