...
**Last updated:** 2026-09-02 — **v1.6.0 released**; **v1.6.1 corrections merged to `develop`, awaiting screenshots before tagging** ([PR #18](https://github.com/rstamps01/ps-deploy-report/pull/18)). **M6 complete (Phases A–E):** ADF [`v0.3.0`](https://github.com/rstamps01/agentic-dev-framework/releases/tag/v0.3.0) ships `framework-doctor` + portfolio PM; portfolio scores 100/100 across vast, planalyzer, and ADF — **all at L3** after the planalyzer pilot ran to completion ([PR #3](https://github.com/rstamps01/planalyzer/pull/3): L3 + vitest coverage ratchet). See [`DECISIONS.md`](DECISIONS.md) / [`development/ADF-EXTRACTION.md`](development/ADF-EXTRACTION.md).
...
| HWL-2 | **Library entries could not outrank broad built-in vendor keys.** Height and image lookups searched the built-in catalog to exhaustion before the user Library, so a bare vendor key (`hpe`) claimed a specific user device (`hpe_turin_cbox`) and rendered it at the wrong U-height, stretching its artwork. | High | **Done** (2026-09-02) | Both catalogs are now matched as one namespace, longest key first. Reported from a real report where a 1U HPE Turin CBox drew 1.8x tall. Regression suite in `tests/test_hardware_library.py`. |
...
1a. **v1.6.1 post-1.6.0 corrections (merged to `develop`, unreleased — [PR #18](https://github.com/rstamps01/ps-deploy-report/pull/18)):** CI action bumps clearing the Node 20 deprecation; `ci.yml` exempts `main` from `cancel-in-progress` so a superseded `develop` run can no longer redden a released commit's banner; shared `version-badge.js` across the marketing site; Output Results expand no longer buries its own menu, and the terminal menu binds before init awaits the backend; deployment-tool controls consolidated into the nav dropdown; architecture-aware macOS download links (arm64/Intel were previously decided by GitHub's asset ordering) plus **Exit & Upgrade**; in-app docs now use `{{APP_VERSION}}`-style tokens with guard tests, and unregistered `.md` links fall back to GitHub. **Not tagged** — held pending screenshots for the Quick Start upgrade steps and the Update Tools walkthrough.
