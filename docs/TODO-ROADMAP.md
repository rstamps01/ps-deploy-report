...
| TP-1 / TP-2 | **CNode Tech-Port → VMS discovery & API tunnel hardening.** TP-1 (commit `7b94453`) widened management-IP parsing to all RFC1918 ranges with interface exclusions and a TCP/443 short-circuit. TP-2 (commit `f49aa2b`) replaced the single-pick name heuristic with empirical TCP/443 candidate probing (Eth base IPs first, IB base IPs next, `:m`/bond aliases last; CGNAT 100.64/10 included) and a `parse_management_ip_candidates` + `_first_443_reachable` helper. Live-validated end-to-end against both an IPoIB cluster (mammoth/Allen Institute, em3 100.64.44.2 wins over bond0:m 172.16.128.4) and an Ethernet cluster (selab-var-202, 10.143.11.202 wins after 192.168.2.2 probes CLOSED). 60/60 tests pass on `tests/test_vms_tunnel.py`; module local coverage 88%. Stacked on `feature/tp-1-techport-discovery`; not pushed, no version bump. Remaining Phase-2 work: UI investigation for RPT-VALIDATION-1 / RPT-VALIDATION-2 (Node User field rendering + Node Password reset on Autofill-off). | In progress |

## Planned — Tech-Port follow-ups

| ID | Item | Priority | Status | Notes |
|------|------|----------|--------|--------|
| SR-1 | **`/api/switches/` direct-cluster-IP call bypasses VMS tunnel on Tech-Port mode.** Surfaced during TP-2 validation on `selab-var-202` (Ethernet cluster). `src/workflows/vnetmap_workflow.py:367` calls `https://{api_host}/api/switches/` where `api_host = self._credentials.get("tunnel_address") or host`; in Tech-Port mode `tunnel_address` is unset on the credentials dict the workflow receives, so it falls back to the user-entered cluster IP (a CNode), gets `Connection refused` on TCP/443, and silently mis-classifies the cluster as InfiniBand → runs `vnetmap.py … -ib` → `ibnetdiscover` fails → workflow falls back to whatever cached vnetmap exists locally. Report still completes only because of the fallback. The VMS API tunnel was already up at `127.0.0.1:<port>` at this point (TP-2 established it correctly). Wire `tunnel_address` from `VMSTunnel.connect()` into the credentials passed to `VnetmapWorkflow` and any other workflow that hits the cluster API directly. Out of scope for TP-1 / TP-2. Full repro evidence: `import/cluster202/output-logs-043026` lines 56, 62, 83, 182–183, 191, 252, 478. Tracking doc: `docs/issues/SR-1/01-summary.md`. | Medium | Planned | Pre-existing; not introduced by TP-1/TP-2; affects accuracy of cluster-type detection and freshness of port mapping data on Tech-Port runs. |
