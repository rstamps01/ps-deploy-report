...
@pytest.fixture
def mapper(tmp_path):
    with patch("external_port_mapper.VerboseLogger") as mock_vlog_cls, \
         patch("utils.get_data_dir", return_value=tmp_path), \
...
class TestSwitchDetection:
    ...
    def test_detect_onyx_switch(self, mapper):
        with patch("external_port_mapper.run_ssh_command") as mock_ssh, \
             patch("external_port_mapper.run_interactive_ssh") as mock_issh:
...
class TestCorrelation:
    def test_correlate_node_to_switch(self, mapper):
        ...
        switch_macs = {
            "10.0.0.10": {"aa:bb:cc:dd:ee:01": {"port": "swp1", "vlan": "1"}}
        }
        result = mapper._correlate_node_to_switch(
    ...
    def test_collect_port_mapping_integration(self, mapper):
        with patch.object(mapper, "_detect_switch_os") as mock_detect, \
             patch.object(mapper, "_collect_node_inventory_basic_auth") as mock_inv, \
             patch.object(mapper, "_collect_ebox_mapping") as mock_ebox, \
             patch.object(mapper, "_collect_hostname_to_ip_mapping") as mock_host, \
             patch.object(mapper, "_collect_node_macs_via_clush") as mock_macs, \
             patch.object(mapper, "_collect_switch_mac_tables") as mock_switch, \
             patch.object(mapper, "_collect_ipl_connections") as mock_ipl, \
             patch("builtins.print"):
            ...
            mock_macs.return_value = {
                "172.16.0.1": {"enp129s0f0": "aa:bb:cc:dd:ee:01"}
            }
            mock_switch.return_value = {
                "10.0.0.10": {
                    "aa:bb:cc:dd:ee:01": {"port": "swp1", "vlan": "1"}
                }
            }
