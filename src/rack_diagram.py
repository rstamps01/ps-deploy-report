...
class RackDiagram:
    ...
    def _load_hardware_images(self) -> Dict[str, Optional[Path]]:
        ...
        image_map = {
            ...
            "dbox-515": HARDWARE_IMAGE_DIR / "ceres_v2_1u.png",  # Map dbox-515 to ceres_v2 image
            "msn3700-vs2fc": HARDWARE_IMAGE_DIR / "mellanox_msn3700_1x32p_200g_switch_1u.png",  # Mellanox switch
    ...
    def _calculate_switch_positions(
        self, cboxes: List[Dict[str, Any]], dboxes: List[Dict[str, Any]], num_switches: int
    ) -> List[int]:
        """
        ...
        if not cbox_positions or not dbox_positions:
            logger.warning("Cannot calculate switch positions: insufficient CBox or DBox data")
            return []
        ...
        if gap_top < gap_bottom:
            logger.warning(f"No gap between CBoxes and DBoxes: CBox bottom={lowest_cbox}, DBox top={highest_dbox}")
            return []
        
        # Calculate gap size
        gap_size = gap_top - gap_bottom + 1  # Inclusive count
        
        logger.info(f"Gap between CBoxes and DBoxes: {gap_size}U (U{gap_bottom} to U{gap_top})")
        ...
        if gap_size % 2 == 0:
            ...
            # Example: 4U gap (U11-U14): place at U13, U12
            center_top = gap_bottom + (gap_size // 2)
            center_bottom = center_top - 1
            switch_positions = [center_top, center_bottom]
            logger.info(f"Even gap ({gap_size}U): Placing switches at U{center_top} and U{center_bottom}")
        else:
            # Odd gap: Place switches in center with 1U gap between them
            # Example: 5U gap (U11-U15): place at U14, U12 (leaving U13 empty)
            center_u = gap_bottom + (gap_size // 2)
            switch_top = center_u + 1
            switch_bottom = center_u - 1
            switch_positions = [switch_top, switch_bottom]
            logger.info(f"Odd gap ({gap_size}U): Placing switches at U{switch_top} and U{switch_bottom} (U{center_u} empty)")
    ...
    def generate_rack_diagram(
        ...
        if switches and len(switches) > 0:
            calculated_positions = self._calculate_switch_positions(cboxes, dboxes, len(switches))
