...
class RackDiagram:
    ...
    def __init__(self, page_width: float = 7.5 * inch, page_height: float = 9.5 * inch):
        """
        Initialize rack diagram generator.

        Args:
            page_width: Available page width in points (default 7.5" for content area)
            page_height: Available page height in points (default 9.5" for content area)
        """
        ...
        self.available_height = page_height - (1.5 * inch)  # Space for labels
        ...
        self.scale = min(width_scale, height_scale) * 0.75  # 75% to leave room for labels
    ...
    def _parse_rack_position(self, position: str) -> int:
        ...
        try:
            ...
            if 1 <= u_number <= 42:
                ...
                logger.warning(f"Invalid rack position {position}, must be between U1-U42")
                return 0
        except ValueError:
    ...
    def _create_device_representation(
        ...
        # Add device icon (centered)
        icon_x = start_x + (self.rack_width / 2)
        icon_y = device_y + (device_height / 2) - 4

        if device_type.lower() == "cbox":
            # Compute icon (simplified CPU representation)
            icon_text = "CPU"
        elif device_type.lower() == "dbox":
            # Storage icon (cylinder representation)
            icon_text = "HDD"
        elif device_type.lower() == "switch":
            # Network icon
            icon_text = "NET"
        ...
        label_x = start_x + self.rack_width + 0.2 * inch
    ...
    def generate_rack_diagram(
        ...
        for cbox in cboxes:
            ...
            self._create_device_representation(
                drawing, "cbox", device_id, u_position, u_height, status
            )
        ...
        for dbox in dboxes:
            ...
            self._create_device_representation(
                drawing, "dbox", device_id, u_position, u_height, status
            )
